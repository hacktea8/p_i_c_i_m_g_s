...
class Imagelib{
  ...
  function init($imageaddress, $types, $value1="", $value2="", $endaddress){
		$this->types=$types;
  ...
  if($this->imagesizex() < $imagearrs[0] || $this->imagesizey() < $imagearrs[1]){
    return false;
  }
		//调用函数计算出水印加载的位置
	...
	private function position($num, $width, $height){//根据传入的数字返回一个位置的坐标,$width和$height分别代表插入图像的宽和高
		switch($num){
			...
			case 8:
				...
				break;
			case 9:
				$positionarr[0]=$this->width-$width - 12;
				$positionarr[1]=$this->height-$height -12;
				break;
			case 0:
